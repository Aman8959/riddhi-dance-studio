...
function submit(body) {
  if (!["registration", "trial", "contact", "review"].includes(body.type))
...
function login(body) {
  ...
  if (email !== configuredEmail || body.password !== properties.getProperty("ADMIN_PASSWORD"))
...
function listApprovedReviews() {
  return {
    ...
    data: getSheet()
      ...
      .slice(1)
      .filter(function (row) {
        return String(row[1]).trim() === "review" && String(row[2]).trim() === "confirmed";
      })
      .map(function (row) {
...
function updateStatus(body) {
  ...
  if (!["new", "contacted", "confirmed", "rejected"].includes(body.status))
...
function addMedia(body) {
  ...
  const media = {
    ...
    url: "",
    thumbnailUrl: "",
    youtubeId: body.youtubeId ? normalizeYoutubeId(body.youtubeId) : "",
  ...
  if (body.kind === "video") {
    ...
    if (body.file) {
      const file = saveMediaFile(body.file, media.title);
      media.driveId = file.id;
      media.url = file.url;
    }
    if (body.thumbnail) {
      const thumbnail = saveMediaFile(body.thumbnail, media.title + " thumbnail");
      media.thumbnailDriveId = thumbnail.id;
    ...
    media.driveId = file.id;
    media.url = file.url;
    media.thumbnailUrl = file.url;
  }
  getMediaSheet().appendRow([
    media.id,
    media.kind,
    media.title,
    media.category,
...
function deleteMedia(body) {
  ...
  trashDriveFile(values[rowIndex][8]);
  trashDriveFile(values[rowIndex][9]);
...
function saveContent(body) {
  requireAdmin(body.token);
  if (!CONTENT_TYPES.includes(body.type) || !body.item || typeof body.item !== "object")
  ...
  if (body.item.file) {
    const file = saveMediaFile(
      body.item.file,
      String(body.item.name || body.item.className || "content image"),
    );
    item.image = file.url;
    delete item.file;
  }
...
function deleteContent(body) {
  ...
  const rowIndex = sheet
    ...
    .findIndex(function (row) {
      return String(row[0]) === String(body.id);
    });
...
function getContentSheet(type) {
  ...
  const sheet =
    spreadsheet.getSheetByName("Content_" + type) || spreadsheet.insertSheet("Content_" + type);
...
function saveMediaFile(input, title) {
  ...
  const file = getMediaFolder()
    .createFile(Utilities.newBlob(bytes, input.mimeType, input.name))
    .setName(title + " - " + input.name);
...
function publicMedia(media) {
  const fileUrl = media.driveId
    ? "https://drive.google.com/thumbnail?id=" + media.driveId + "&sz=w1600"
    : media.url;
  const thumbnailUrl = media.thumbnailDriveId
    ? "https://drive.google.com/thumbnail?id=" + media.thumbnailDriveId + "&sz=w1600"
    : media.thumbnailUrl ||
      (media.kind === "video" && media.youtubeId
...
function requireAdmin(token) {
  ...
  if (!cacheValid && (storedToken !== token || expiresAt < Date.now()))
...
function getSheet() {
  ...
  let spreadsheet;
  try {
    spreadsheet = SpreadsheetApp.openById(id.trim());
  } catch (error) {
    throw new Error(
      "Invalid SHEET_ID. Use the ID from https://docs.google.com/spreadsheets/d/SHEET_ID/edit, not an Apps Script library or Web App ID.",
    );
  }
...
function getMediaFolder() {
  ...
  try {
    ...
    throw new Error("Invalid MEDIA_FOLDER_ID. Use the ID from the media folder URL.");
  }
}
