...
function MediaManager({ token, media, onMediaChange }: { token: string; media: MediaItem[]; onMediaChange: (items: MediaItem[]) => void }) {
  ...
  async function publish(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); setSaving(true); try { const item = await addMedia(token, { kind, title, category, youtubeId: kind === "video" ? youtubeId : undefined, file: kind === "video" ? undefined : await payload(file), thumbnail: kind === "video" ? await payload(thumbnail) : undefined }); onMediaChange([item, ...media]); setTitle(""); setYoutubeId(""); event.currentTarget.reset(); toast.success("Media published."); } catch (error) { toast.error(error instanceof Error ? error.message : "Could not publish media."); } finally { setSaving(false); } }
